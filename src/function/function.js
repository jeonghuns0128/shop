...
import { MypageMiddleMenu, IndividualIntervalsMyPage, FlushMyPage } from '../pages/mypage'
...
function HistoryBack(){
    ...
    return (
        <>
            <MdArrowBackIosNew size='30' style={{color : 'white'}} onClick={() => {navigate(-1)}} />
        </>
    )
...
function MyPage() {
    ...
    useEffect(() => {
        ...
        if(result.data.passport.user.id){
            setUser(result.data.passport.user.id)
            setIsLogin(true)
            setAccessToken(result.data.passport.user.accessToken)
        }
        })
        .catch(() => {
            console.log('실패')
        ...
        <div>
            {
                ...
                <div style={{width : '70%', height : '50px', float : 'left'}}>
                    <div style={{textAlign :'left', paddingLeft : '10px'}}> {user}님 안녕하세요! </div>
                ...
                }}><SlLogout size="30px" /><br/>&nbsp;&nbsp;로그아웃</div>
                </div>
            }
    ...
    return (
        <div>
            ...
            <div style={{textAlign : 'center', marginTop : '10px'}}>
                <a href="">
                    <img src={process.env.PUBLIC_URL + './naver_login.png'} style={{width : '300px', height : '45px'}} />
                </a>
            </div>
        </div>
    )
